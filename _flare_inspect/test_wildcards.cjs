const { createClient } = require('@supabase/supabase-js');
const s = createClient('https://iqkgfqygpjzsbklgywsw.supabase.co', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imlxa2dmcXlncGp6c2JrbGd5d3N3Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MTA1MzU5MiwiZXhwIjoyMDg2NjI5NTkyfQ.sIMozyTig2cEnSGYcS_hQOgt4oghiWNgZhdkMEtdfsE');

async function testOr() {
    // Test with %
    const { data: resultsPercent, error: errorPercent } = await s
        .from('users')
        .select('name')
        .or('name.ilike.%ar%')
        .limit(5);
    console.log("Results with %:", resultsPercent?.length || 0, resultsPercent);

    // Test with *
    const { data: resultsStar, error: errorStar } = await s
        .from('users')
        .select('name')
        .or('name.ilike.*ar*')
        .limit(5);
    console.log("Results with *:", resultsStar?.length || 0, resultsStar);

    process.exit(0);
}
testOr();
