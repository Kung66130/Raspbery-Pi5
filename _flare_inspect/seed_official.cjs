...
async function seed() {
    ...
    // Create the user
    const { error: userErr } = await s.from('users').upsert([{
        id: officialId,
    ...
    else console.log('User Flare Official created');

    // Create the post
    const { error: postErr } = await s.from('posts').upsert([{
    ...
    else console.log('Official Post created successfully!');
